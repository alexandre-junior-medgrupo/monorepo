 ...
 */
const distribution = () => {
    printWelcomeMessage("Compila os pacotes escolhidos e move os compilados para a pasta 'dist' no 'root' do monorepo.");
    const inquire = (answer) => {
        const { inquire } = answer;
    ...
    const inquire = (answer) => {
        ...
        if (inquire.includes('templarios-core')) {
            inquirer
                ...
                const { inquire2 } = answer2;
                const ionicRelease = inquire2.slice(-1);
                execSync(`lerna run node:replace:package:build:${ionicRelease} --scope=templarios-core`, { stdio: 'inherit' });
                execSync(`lerna run build ${inquire.length > 1
                    ? `--parallel --scope={${inquire}}`
                    : `--scope=${inquire}`}`, { stdio: 'inherit' });
            })
                .catch((error) => log(error));
...
program
    ...
    .option('-dist, --distribution', "compila os pacotes escolhidos e move os compilados para a pasta 'dist' no 'root' do monorepo.")
    .action((option) => {
    if (option.start) {
        start();
    }
